...
export function AppointmentCard({ appointment, contact, onClick, compact = false }: { appointment: Appointment; contact?: Contact; onClick: () => void; compact?: boolean }) {
  return <button className={`appointment-card ${compact ? 'compact' : ''} status-${appointment.status}`} type="button" onClick={onClick}>
    <span className="date-tile"><small>{new Intl.DateTimeFormat('en-CA', { month: 'short' }).format(new Date(`${appointment.date}T00:00:00`))}</small><strong>{appointment.date.slice(-2).replace(/^0/, '')}</strong></span>
