...
export function AppointmentDetail({ appointment, doctor, adminMode, onBack, onEdit, onCancel, onDelete }: Props) {
  const mapsHref = appointment.mapsUrl || (appointment.address ? `https://maps.google.com/?q=${encodeURIComponent(appointment.address)}` : '')
  const phone = appointment.phone || doctor?.phone || ''
  return (
    <article className="detail-page">
      ...
      <section className="detail-hero">
        ...
        <p className="eyebrow">{relativeDate(appointment.date)}</p>
        <h1>{appointment.purpose}</h1>
        <p className="detail-date">{formatLongDate(appointment.date)}</p>
        <p className="detail-time">{formatTimeRange(appointment.startTime, appointment.endTime)}</p>
      </section>
      <section className="detail-card">
        {doctor && <div className="detail-row"><span className="detail-icon"><Icon name="user" /></span><div><small>Care provider</small><strong>{doctor.name}</strong><p>{doctor.specialty}</p></div></div>}
        ...
        {(mapsHref || phone) && <div className="detail-actions">
          ...
          {phone && <a className="button secondary" href={`tel:${phone.replace(/[^+\d]/g, '')}`}><Icon name="phone" />Call Clinic</a>}
        </div>}
      </section>
