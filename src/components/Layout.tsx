...
const navItems: Array<{ view: AppView; label: string; icon: 'home' | 'calendar' | 'contacts' }> = [
  { view: 'home', label: 'Home', icon: 'home' },
  { view: 'calendar', label: 'Calendar', icon: 'calendar' },
  { view: 'contacts', label: 'Contacts', icon: 'contacts' },
]
...
export function BottomNavigation({ current, onChange }: { current: AppView; onChange: (view: AppView) => void }) {
  return <nav className="bottom-nav" aria-label="Main navigation">
    {navItems.map((item) => <button key={item.view} className={current === item.view ? 'active' : ''} type="button" onClick={() => onChange(item.view)} aria-current={current === item.view ? 'page' : undefined}>
...
export function PageIntro({ eyebrow, title, text, action }: { eyebrow?: string; title: string; text?: string; action?: ReactNode }) {
  return <div className="page-intro"><div>{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h1>{title}</h1>{text && <p>{text}</p>}</div>{action}</div>
}
